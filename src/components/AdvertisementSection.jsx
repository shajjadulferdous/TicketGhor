...
import { MdAirlineSeatReclineNormal, MdLocalOffer } from 'react-icons/md';
...
export default async function AdvertisementSection() {
  ...
  try {
    // Note: We are fetching all tickets and filtering here. 
    // For a massive app, you'd want a specific backend route like /tickets/advertised
    const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/tickets`, {
      next: { revalidate: 60 } // Revalidates the cache every 60 seconds so new ads show up quickly
    });
    ...
    if (!res.ok) {
      ...
      // Filter for advertised tickets and enforce the EXACTLY 6 rule
      advertisedTickets = allTickets
        .filter(ticket => ticket.isAdvertised && ticket.status === 'approved')
        .slice(0, 6); 
    }
  } catch (error) {
  ...
  // If there's an error or no tickets are currently advertised, we can just hide the section entirely
        ...
        <div className="mb-10 text-center md:text-left flex flex-col md:flex-row justify-between items-end gap-4">
          <div>
            <div className="flex items-center justify-center md:justify-start gap-2 mb-2">
              <MdLocalOffer className="text-[#35858E]" size={24} />
            ...
            >
              ...
              <div className="p-6 flex flex-col flex-grow">
                ...
                <div className="mt-auto pt-2">
                  <Link 
                    href={`/tickets/${ticket._id}`} // Make sure this matches your actual details page route!
